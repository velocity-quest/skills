...
You have access to the Velocity project management MCP server. Use it **proactively** during development to track your work, file bugs, update issue statuses, and add comments — not just when the user explicitly asks.
...
add_comment(issueId: "...", body: "Blocked: need decision on whether to use WebSockets or SSE.")
```
...
| `list_members` | — | — |
| **Roadmap** | | |
| `list_roadmap_items` | — | `status`, `category` (FEATURE/IMPROVEMENT/BUG/INFRASTRUCTURE), `versionId`, `limit` |
